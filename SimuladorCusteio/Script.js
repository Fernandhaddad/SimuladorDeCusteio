...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        ...
        // =================================================================
        // 6. EVENT LISTENER (Ponto de Entrada) - MOVIDO PARA AQUI
        // Este listener agora só é anexado APÓS todos os dados serem carregados com sucesso.
        // =================================================================
        ...
        if (calcularRotaBtn) {
            ...
            // Este alerta pode indicar um problema no HTML, como um ID incorreto.
        ...
        // Exibe erro de carregamento na div de resultados também
...
// =================================================================
// NOTA: A inicialização do mapa pode ser feita imediatamente ao carregar o DOM
// pois não depende diretamente dos dados JSON para funcionar, apenas para exibir a rota.
...
async function calculateAndDisplayRoute() {
    ...
    // Limpa resultados anteriores e esconde avisos
    // Seleciona os elementos de resultado pelo ID
    ...
    const loadingMessageElem = document.getElementById('loading-message'); // Adicionado o elemento de loading
    const errorDisplayMessageElem = document.getElementById('error-display-message'); // Adicionado o elemento de erro

    // Oculta todos os blocos de resultado e limpa o conteúdo no início
    loadingMessageElem.style.display = 'none';
    avisoMultiViagemElem.style.display = 'none';
    distanciaVeiculoElem.style.display = 'none';
    custoAnttValorElem.parentElement.style.display = 'none'; // Esconde o bloco pai do custo ANTT
    custoNtcValorElem.parentElement.style.display = 'none'; // Esconde o bloco pai do custo NTC
    duracaoTotalValorElem.parentElement.style.display = 'none'; // Esconde o bloco pai da duração
    errorDisplayMessageElem.style.display = 'none';

    // Limpa o conteúdo
    distanciaVeiculoElem.innerHTML = '';
    custoAnttValorElem.textContent = '';
    custoAnttDetalheElem.innerHTML = '';
    custoNtcValorElem.textContent = '';
    custoNtcDetalheElem.innerHTML = '';
    duracaoTotalValorElem.textContent = '';
    duracaoTotalDetalheElem.innerHTML = '';
    ...
    // A lógica de múltiplas viagens usa `capacidadeToneladas` (carga útil)
    if (pesoCarga > veiculoSelecionado.capacidadeToneladas) {
        ...
        avisoMultiViagemElem.style.display = 'block'; // Mostra o aviso
    }
    ...
    try {
        ...
        // --- Exibição dos Resultados (Preenchendo os elementos HTML e mostrando-os) ---
        routeLayer.clearLayers();
        loadingMessageElem.style.display = 'none'; // Esconde a mensagem de carregamento
        ...
        custoAnttValorElem.parentElement.style.display = 'block'; // Mostra o bloco pai do custo ANTT
        ...
        `;
        custoNtcValorElem.parentElement.style.display = 'block'; // Mostra o bloco pai do custo NTC
        ...
        `;
        duracaoTotalValorElem.parentElement.style.display = 'block'; // Mostra o bloco pai da duração
        ...
        L.marker([destinoInfo.lat, destinoInfo.lon]).addTo(routeLayer).bindPopup(`<b>Chegada:</b><br>${destinoInfo.address || destinoAddressText}`); // Usar addressdetails se disponível
        map.fitBounds(routeLine.getBounds());

    } catch (error) {
        // Em caso de erro, esconde o loading e exibe a mensagem de erro
        loadingMessageElem.style.display = 'none';
